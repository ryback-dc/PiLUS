...
class Customer extends Plbase 
{
	...
	public function addCustomer()
	{
		...
		$sql = "INSERT INTO pl_customers(fullname, email, password, 
				address, phone, district_id, shipping_id, 
				customer_type, customer_session, date_registered, 
				time_registered)VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

		$customerSesi = generateSessionKey($this->customer_session);
		
		$data = array($this->fullname, $this->email, $this->password,
				$this->address, $this->phone, $this->district_id, 
				$this->shipping_id, $this->customer_type, $customerSesi, 
				$this->date_registered, $this->time_registered);
	...
	public function registerMember()
	{
		...
		if ( isset($id_member))
		{
			...
			$getMember = "SELECT ID, fullname, email, password
						  FROM pl_customers WHERE email = :email";
			
			$stmt = $dbh -> prepare($getMember);
			$stmt -> bindValue(":email", $this->email, PDO::PARAM_INT);
			
			$stmt -> execute();
			$data_member = $stmt -> fetchObject();
			
			$ID = $data_member -> ID;
			$nama_member = $data_member -> fullname;
			$email_member = $data_member -> email;
			$alamat_member = $data_member -> address;
	...
	public  function emailExists($email)
	{
		...
		$sql = "SELECT COUNT(`ID`) FROM pl_customers 
