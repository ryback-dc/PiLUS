...
class Order 
{
	 ...
	 * table pl_orders
	 */
	public function simpanTransaksi($nama, $alamat, $telpon, $email, $password, $district_id, $shipping_id, $customer_type)
	{
		...
		$nama_kustomer = preventInject($nama);
		$email_kustomer = safeEmail($email);
		$alamat = preventInject($alamat);
		$kata_sandi = trim($password);
		$phone = preventInject($telpon);
		$kota = (int)$district_id;
		$kurir = (int)$shipping_id;
		...
		$data_customer = array(
				...
				'fullname' => $nama_kustomer,
				'email' => isset($email) ? preg_replace('/[^ \@\.\-\_a-zA-Z0-9]/', '', $email) : '',
				'password' => $kata_sandi,
				'address' => $alamat,
				'phone' => $phone,
				'district_id' => $kota,
				...
				'customer_session' => generateSessionKey($kata_sandi),
				'date_registered' => date("Y-m-d"),
				'time_registered' => date("H:i:s")
		);

		$kustomer_baru = new Customer($data_customer);
		$id_kustomer = $kustomer_baru -> addCustomer();
		...
		$ongkos = $shippingCost -> getDistrictById($district_id);
