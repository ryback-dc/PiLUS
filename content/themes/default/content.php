...
function checkOut() 
{
	...
	else
	{
		...
		$html[] = '<div class="col col_13 checkout">';
		$html[] = '*Nama Lengkap:
				  <input type="text" name="name" style="width:300px;" maxlength="150"  />
				  </div>';
		// alamat pengiriman
		$html[] = '<div class="col col_13 checkout">';
		...
		$html[] = '<p><input type="checkbox" name="customer_type" value="member" />Belanja sekaligus daftar member</p>';
		$html[] = '<p><input type="checkbox" name="customer_type" value="guest" />Belanja saja</p>';
		$html[] = '</div>';
...
if (!$customer -> isMemberLoggedIn()) 
{
	if (isset($_POST['login']) && $_POST['login'] == 'Masuk')
	{
		...
		if (empty($email) || empty($password)) // cek kolom pengisian email dan password
		{
			$html[] = '<h2>Error</h2>
					<div class="cleaner"></div>
					<blockquote>
		...
		elseif (strlen($email)) // cek validitas penulisan email
		{
...
// ***************Konten simpan transaksi pelanggan baru******************** //
function saveTransaction() 
{
	...
	if (isset($_POST['send']) && $_POST['send'] == 'Proses')
	{
		$nama_lengkap = isset($_POST['name']) ? filter_input(INPUT_POST, 'name', FILTER_SANITIZE_STRING) : "";
		$alamat = isset($_POST['address']) ? $_POST['address'] : "";
		$notelp = isset($_POST['phone']) ? str_replace(array(' ', '-', '(', ')'), '', $_POST['phone']) : "";
		$email = isset($_POST['email']) ? filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL) : "";
		$password = isset($_POST['password']) ? trim($_POST['password']) : "";
		$confirmed = isset($_POST['confirmed']) ? trim($_POST['confirmed']) : "";
		$district_id = isset($_POST['city']) ? (int)$_POST['city'] : 0;
		$shipping_id = isset($_POST['shipping']) ? (int)$_POST['shipping'] : 0;
		$customer_type = isset($_POST['customer_type']) ? htmlspecialchars($_POST['customer_type']) : "";
		
		if ( empty($nama_lengkap) || empty($alamat) || empty($notelp)
		   ||  empty($email) || empty($password) || empty($confirmed) 
		   || $district_id == 0 || $shipping_id == 0 || empty($customer_type))
		{
			$html[] = '<h2>Error</h2>
					<div class="cleaner"></div>
					<blockquote>
		...
		// cek nama lengkap
		elseif (!preg_match('/^[A-Z \'.-]{2,90}$/i', $nama_lengkap))
		{
			$html[] = '<h2>Error</h2>
					<div class="cleaner"></div>
					<blockquote>
		...
		// cek alamat
		elseif (!preg_match('/^[A-Z0-9 \',.#-]{2,255}$/i', $alamat))
		{
			$html[] = '<h2>Error</h2>
					<div class="cleaner"></div>
					<blockquote>
		...
		// cek no_telpon
		elseif (!preg_match('/^[0-9]{10,13}$/', $notelp))
		{
			$html[] = '<h2>Error</h2>
					<div class="cleaner"></div>
					<blockquote>
		...
		elseif ( is_numeric($password) == true)
		{
			$html[] = '<h2>Error</h2>
					<div class="cleaner"></div>
					<blockquote>
		...
		else
		{
			...
			$transaksi_baru = $order -> simpanTransaksi($nama_lengkap, $alamat, $notelp, $email, $password, $district_id, $shipping_id, $customer_type);
	...
	try {
		...
		else 
		{
		  ...
		  while ($row = $sth -> fetch()) {
		   ...
		   $html[] = $pages -> page_links('blog'.$_GET['blogid'].'&'); // Blog pagination
...
function blogDetail() 
{
	...
	elseif (isset($_POST['send']) && $_POST['send'] == 'Kirim')
	{
		...
		if (empty($_POST['nama_komentar']) || empty($_POST['isi_komentar']) || empty($_POST['captcha']))
		{
			$html[] = '<blockquote>
				     <h3>Terjadi Kesalahan!</h3>
					   Semua kolom bertanda asterik(*) harus diisi.
...
function editProfile() 
{
	...
	if ( isset($_POST['saveChange']) && $_POST['saveChange'] == 'Simpan') {
		...
		} elseif (!preg_match('/^[A-Z \'.-]{2,90}$/i', $_POST['fullname'])) {
		...
		} elseif (valid_email($_POST['email']) == false) {
		...
		if ( isset($data_member) && $data_member -> getId() !== $_SESSION['member_id']) {
			...
			$html[] = '<label for="address">Alamat:</label>
				  <input type="text" id="address" name="address" class="required input_field" value="'.htmlspecialchars($data_member -> getCustomerAddress() ).'" />
				  <div class="cleaner h10"></div>';
...
function memberLogin() 
{
	...
	if (isset($_POST['login']) && $_POST['login'] == 'Masuk') {
		...
		} elseif (valid_email($email)) { // cek validitas penulisan email
